...
from medusa.exceptions import QueueException


class TaskRegistry(object):
    """
    A simple Registry used to track subclasses of :class:`QueueTask`.
    The purpose of this registry is to allow translation from queue
    messages to task classes, and vice-versa. Task serialization from
    queue messages to classes and vice-versa.
    """
    ...
    def register(self, task_class):
        ...
        if klass_str not in self._registry:
            ...
            # store an instance in a sepaarte list of periodic tasks
    ...
    def get_task_class(self, klass_str):
        ...
        if not klass:
            raise QueueException("{} not found in TaskRegistry".format(klass_str))
    ...
    def get_task_for_message(self, msg):
        ...
        klass = self.get_task_class(klass_str=klass_str)
